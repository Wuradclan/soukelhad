...
// Configuration pour Next.js 15+ : les props sont des Promises
export default async function UserDashboard(props: {
  searchParams: Promise<{ success?: string; error?: string }>;
}) {
  // 1. On attend les paramètres d'URL (obligatoire en Next.js 15)
  const searchParams = await props.searchParams;

  // 2. Initialisation de Supabase côté serveur
  ...
  // NOTE : J'ai retiré 'ig_username' de la requête pour éviter les erreurs 
  // si la colonne n'existe pas encore dans ta table 'shops'.
  const { data: shop, error: shopError } = await supabase
    .from('shops')
    .select('name, slug, ig_access_token')
    .eq('user_id', user.id)
    .maybeSingle(); // Plus souple que .single() pour éviter les erreurs 406

  if (shopError) {
    console.error("Erreur lors de la récupération de la boutique :", shopError.message);
  }
          ...
          <div className="mb-8 p-5 bg-rose-50 border border-rose-100 text-rose-800 rounded-2xl flex items-center gap-4">
             ...
             <p className="font-bold text-sm">
                {searchParams.error === 'auth_failed' 
                  ? "La connexion Instagram a échoué. Assurez-vous d'utiliser un compte Professionnel." 
            ...
            <div>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-2xl font-black text-slate-800">{shop?.name || "Boutique en attente de configuration"}</p>
                </div>
                ...
                <div>
                  ...
                  {shop?.slug ? (
                    ...
                    <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl text-amber-700 text-sm font-medium">
                      Votre lien personnalisé n'est pas encore prêt.
                    </div>
                  )}
                </div>
              </div>
            ...
            {shop?.ig_access_token ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm text-slate-400 mt-2 font-medium leading-relaxed">
                  Vos produits sont synchronisés.<br/>Toute nouvelle photo Instagram apparaîtra ici.
                </p>
                <button className="mt-10 text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-rose-500 transition-colors">
